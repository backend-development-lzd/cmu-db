...
class TrieNode {
 public:
  ...
  bool HasChild(char key_char) const {
    if(children_.find(key_char)!=children_.end()) return true;
    else return false;
  }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    auto term_node = node->get()->GetChildNode(ch);
    ...
    if(term_node== nullptr){
      ...
      auto new_term_node = new TrieNodeWithValue<T>(std::move(*(term_node->get())),value);
      term_node->reset(new_term_node);
      ret=true;
    }else{
      ret= false;
    }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto term_node = dynamic_cast<TrieNodeWithValue<T>*>(node->get());
    T ret;
    if(term_node!= nullptr){
      *success = true;
